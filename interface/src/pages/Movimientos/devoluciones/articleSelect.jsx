...
const fetchArticles = async ({pk,query}) => {
  try {
    const response = await instance.get(`/articulos/res/${pk}/${encodeURIComponent(query)}`);
...
const ViewRestok_ArticleSelect = () => {
  ...
  useEffect(() => {
    const loadArticles = async () => {
      setError(null);
      try {
        const data = await peticiones.fetchArticles({pk:pkResponsable,query:query});
