...
const peticion = () => {
  ...
  const Buscar = async ({ query }) => {
    try {
      const response = await instance.get(`/${section}/responsables/search/${encodeURIComponent(query)}`);
            ...
            {data.map((element,index) => 
              ((index <= limit.max && index >= limit.min) && (
                <Componentes.Table.columna key={element.pk}>
                  <Componentes.Table.fila children={element.nombres}/>
                  <Componentes.Table.fila children={element.correo}/>
                  <Componentes.Table.fila children={element.Asignaciones.length}/>
